...
echo "$DEVELOPER_ID_APPLICATION_CERT" | base64 -d > developer-id-application-cert.p12
openssl pkcs12 -in developer-id-application-cert.p12 -legacy -passin pass: -nodes -nocerts | openssl rsa -out developer-id-application-cert.key

app-store-connect fetch-signing-files app.bubbletrail.bubbletrail \
    --platform MAC_OS \
    --type MAC_APP_DIRECT \
    --certificate-key=@file:developer-id-application-cert.key \
    --create
